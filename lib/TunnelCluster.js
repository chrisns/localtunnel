...
var WhiteListTransformer = require('./WhitelistTransformer');
...
TunnelCluster.prototype.open = function() {
    ...
    function conn_local() {
        ...
        local.once('connect', function() {
            ...
            try {
                stream = stream.pipe(WhiteListTransformer({subnet: opt.whitelist_subnet, path: opt.whitelist_path}));
                stream = stream.pipe(local).pipe(remote);
            }
            catch (e) {
                local.end()
            }
